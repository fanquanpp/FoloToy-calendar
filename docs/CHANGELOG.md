...
- 全仓库文档语言翻转：所有 Markdown 默认 `.md` 文件改为简体中文，英文版本使用配对的 `.en.md`（替换原有 `X.zh_CN.md` 英文默认方案）；批量更新文件重命名、切换链接与全文交叉引用，并同步 `tools/check_repo.py` 的语言配对校验与 `AGENTS`/`doc-conventions` 语言规范。
...
- 修复低功耗唤醒源误报：`demo_low_power` 仅在本次确实启用了 RTC 定时器唤醒时才调用 `esp_sleep_disable_wakeup_source`，消除进入 `Low Power` 页面直接退出时打印的 `E sleep: Incorrect wakeup source (0x4) to disable`；真机验证「进入即退出 / LIGHT SLEEP / DEEP SLEEP」三个场景，light/deep sleep 计时唤醒链路均正常且不再报该 E 日志。

## 0.1.0

- 新增日历玩法：公历月历网格、倒计时目标、每年纪念日高亮、今天/目标分色、静音优先设计，以及空闲自动熄背光的低功耗续航优化。
- 新增纯 C 日期逻辑（`main/calendar_logic.c`）与宿主单元测试（`tests/test_calendar_logic.c`），并在演示菜单注册日历入口。
- 发布合并固件 `build/FoloToy-AI-Passport-full.bin`，并补充根目录 README 说明构建、刷写与分享方式。

## Unreleased

- 精简仓库根目录：将 GitHub 可识别的社区治理文档迁入 `.github/`，将变更记录迁入 `docs/`，同步全部引用，并在仓库检查中加入根目录文档白名单。
- 全仓库文档语言规范：所有维护中的 Markdown 默认 `.md` 文件使用英文，简体中文使用配对的 `.md`，双方提供语言切换；静态检查会阻止缺失配对、缺失切换链接或英文默认页混入中文正文。
...
- 文档去 AI 化：`docs/README.md` / `docs/README.md` 移除 AI 专属章节（Entry point、Source-of-truth、提需求格式、BSP 边界、Runtime invariants、验收交付格式、构建命令），README 只保留给人看的项目介绍、硬件能力契约、demo 案例与项目结构；构建命令章节删除（与 `docs/development/build-and-test.md` 重复）。
...
- 文档结构调整：根目录不再放 README——上游英文 README 移入 `docs/README.md`、中文移入 `docs/README.md`（GitHub 从 docs/ 识别主 README）；原 `docs/README.md` 根总索引更名为 `docs/INDEX.md`；同步更新 AGENTS.md / CONTRIBUTING / SUPPORT / fork-guide / doc-conventions 的路径引用。
- 初始化项目文档：新增 `AGENTS.md`、`CLAUDE.md` 和 `CHANGELOG.md`。
- 仓库结构规整：上游英文 `README.md` 更名为 `README.en_US.md`，保留 `README.md`。
...
- 同步更新索引：`docs/software-design/README.md`、`README.en_US.md` / `README.md` 的 `docs/` 目录说明。
...
- 引入社区治理文档（参照 cindy 改写，放仓库根目录）：新增 `CONTRIBUTING.md` / `.md`（贡献指南，针对 ESP-IDF/AI agent/fork 场景改写）、`CODE_OF_CONDUCT.md` / `.md`（贡献者公约）、`SECURITY.md` / `.md`（安全报告流程）、`SUPPORT.md` / `.md`（支持渠道）；AGENTS.md 与 docs/README.md 同步引用。
